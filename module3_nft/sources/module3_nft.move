...
public fun add_metadata<T: store>(nft: &mut WorkshopNFT, key: String, value: T) {
    df::add(&mut nft.id, key, value);
}

/// Remove dynamic metadata from an NFT
public fun remove_metadata<T: store>(nft: &mut WorkshopNFT, key: String): T {
    df::remove(&mut nft.id, key)
}

/// Check if NFT has specific metadata
public fun has_metadata(nft: &WorkshopNFT, key: String): bool {
